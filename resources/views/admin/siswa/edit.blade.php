<div class="w-full mx-auto bg-white shadow-md rounded-lg p-8 mt-10">
    <h1 class="text-2xl font-bold text-gray-700 mb-6">Edit Data Siswa</h1>

    <form action="/admin/kelola-siswa/update/{{ encrypt($siswa->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('put')
        <!-- Nama -->
        <div>
        ...
        <div>
            ...
            <input type="text" name="username" id="username" value="{{ $siswa->username }}" required
                class="mt-1 block w-full rounded-md border border-gray-300 py-1 px-2 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        ...
        <!-- Password -->
        <div>
            <label for="password" class="block text-sm font-medium text-gray-700">Password (Isi jika ingin mengubah password)</label>
        ...
        <div>
            ...
            <input type="text" name="phone" id="phone" value="{{ $siswa->phone ?? '' }}"
                class="mt-1 block w-full rounded-md border border-gray-300 py-1 px-2 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        ...
        <div>
            ...
            @if($siswa->profile)
                <div class="mt-2">
                    <img id="previewOld"
                         src="{{ asset('assets/profil/' . $siswa->profile) }}"
            ...
            @else
                <div class="mt-2">
                    <img id="previewOld"
                         src="{{ asset('assets/profil/user.png') }}"
                         alt="Foto Default"
                         class="w-24 h-24 rounded-full object-cover border border-gray-300 shadow-sm">
                </div>
            @endif
